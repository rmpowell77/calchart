    ...
    , config(config_)
{
    ...
    auto edit_canvas = new wxSplitterWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxSP_3D | wxSP_LIVE_UPDATE);

    auto control_canvas_split = new wxSplitterWindow(edit_canvas, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxSP_3D | wxSP_LIVE_UPDATE);
    control_canvas_split->SetWindowStyleFlag(control_canvas_split->GetWindowStyleFlag() | wxSP_LIVE_UPDATE);
    mControls = new FieldFrameControls(control_canvas_split, config.Get_FieldFrameZoom());

    mCanvas = new FieldCanvas(control_canvas_split, *static_cast<FieldView*>(view), this, config.Get_FieldFrameZoom());
    // set scroll rate 1 to 1, so we can have even scrolling of whole field
    mCanvas->SetScrollRate(1, 1);

    control_canvas_split->SplitHorizontally(mControls, mCanvas);
    auto control_size = mControls->GetSizer()->GetMinSize();
    control_canvas_split->SetSashPosition(control_size.y + 5);
    ...
    //    mSplitter->SetSplitMode(wxSPLIT_VERTICAL);
    //    mSplitter->SetSashPosition(400);

    // Cont editor is the entirtiy
    mContEditor = new ContinuityEditor(static_cast<CalChartDoc*>(GetDocument()), edit_canvas,
        wxID_ANY, wxDefaultPosition, wxSize{ 300, 800 });
    //    mContEditor->SetScrollRate(1, 1);

    edit_canvas->SplitVertically(mContEditor, control_canvas_split);
    edit_canvas->SetSashPosition(mContEditor->GetSizer()->GetMinSize().x + 5);
    ...
    fullSizer->Add(edit_canvas, 1, wxEXPAND);
    SetSizer(fullSizer);
    // re-set the size
    SetSize(size);
    Show(true);
