...
static int GetTextBoxSize(wxDC const& dc, CalChart::DrawableCont const& drawCont, int box_padding, int text_padding)
{
    auto total_size = 0;

    const static auto format_sub_size = dc.GetTextExtent("%@").x;
    for (auto&& i : drawCont.args) {
        total_size += GetTextBoxSize(dc, i, box_padding, text_padding) + text_padding * 2;
    }
...
static void DrawProcCellBox(wxDC& dc, CalChart::DrawableCont const& drawCont, int box_size, int box_padding, int text_padding, int x_start, int y_start, int rounding)
{
    // first draw the box of the total size
    auto fullbox_size = GetTextBoxSize(dc, drawCont, box_padding, text_padding) + 2 * text_padding;
    ...
    while (regex_search(string_to_print, format_match, format_substr)) {
        ...
        text_left += text_size;
        DrawProcCellBox(dc, drawCont.args.at(count), box_size, box_padding, text_padding, text_left, y_start, rounding);
        text_left += GetTextBoxSize(dc, drawCont.args.at(count), box_padding, text_padding) + 2 * text_padding;
        ++count;
...
static void DrawProcCell(wxDC& dc, CalChart::DrawableCont const& drawCont, int cell_size, int box_padding, int text_padding, int x_start, int y_start, int rounding)
{
    DrawProcCellBox(dc, drawCont, cell_size - 2 * box_padding, box_padding, text_padding, x_start + box_padding, y_start + box_padding, rounding);
}
...
void ContinuityEditorCell::DrawToDC(wxDC& dc)
{
    ...
    DrawProcCell(dc, mProc.GetDrawableCont(), Height(), mConfig.Get_ContCellBoxPadding(), mConfig.Get_ContCellTextPadding(), 0, 0, mConfig.Get_ContCellRounding());
}
...
{
    wxMemoryDC temp_dc;
    return GetTextBoxSize(temp_dc, mProc.GetDrawableCont(), mConfig.Get_ContCellBoxPadding(), mConfig.Get_ContCellTextPadding()) + 2 * mConfig.Get_ContCellBoxPadding() + 2 * mConfig.Get_ContCellTextPadding();
}
