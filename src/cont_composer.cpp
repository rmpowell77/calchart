...
class ContComposerPanel : public wxPanel {
    using super = wxPanel;
    DECLARE_CLASS(ContComposerPanel)
    DECLARE_EVENT_TABLE()

public:
    ContComposerPanel(std::unique_ptr<CalChart::ContProcedure> starting_continuity, CalChartConfiguration& config, wxWindow* parent,
        wxWindowID winid = wxID_ANY,
        const wxPoint& pos = wxDefaultPosition,
        const wxSize& size = wxDefaultSize,
        long style = wxTAB_TRAVERSAL | wxNO_BORDER,
        const wxString& name = wxPanelNameStr);
    virtual ~ContComposerPanel() {}
...
private:
    ...
    void OnCmdTextEnterKeyPressed(wxCommandEvent& event);
    void UpdatePanel();
...
BEGIN_EVENT_TABLE(ContComposerPanel, wxPanel)
EVT_TEXT_ENTER(NEW_CONT_BOX, ContComposerPanel::OnCmdTextEnterKeyPressed)
//EVT_COMBOBOX(NEW_CONT_BOX, ContComposerPanel::OnCmdTextEnterKeyPressed)
END_EVENT_TABLE()

IMPLEMENT_CLASS(ContComposerPanel, wxPanel)
...
ContComposerPanel::ContComposerPanel(std::unique_ptr<CalChart::ContProcedure> starting_continuity, CalChartConfiguration& config, wxWindow* parent,
    wxWindowID winid,
    const wxPoint& pos,
    const wxSize& size,
    long style,
    ...
    , mAction([this](CalChart::DrawableCont const& c) { this->OnDrawableContClick(c); })
{
    ...
    mComboSelection = new wxComboBox(
        this, NEW_CONT_BOX, wxEmptyString,
...
const std::pair<std::string, std::function<std::unique_ptr<CalChart::ContProcedure>()> > ContArray[] = {
...
static_assert(sizeof(ContArray) / sizeof(ContArray[0]) == static_cast<int>(ContProc::LAST), "");
...
const ContValue ValueUsageOrder[] = {
    ContValue::N, ContValue::E, ContValue::S, ContValue::W, ContValue::NE, ContValue::SE, ContValue::SW, ContValue::NW,
    ContValue::StepDrill,
    ContValue::Remaining,
    ContValue::Neg,
    ContValue::Add,
    ...
    ContValue::DistFrom,
    ContValue::Opposite,
    ContValue::DOF, ContValue::DOH,
    ContValue::A, ContValue::B, ContValue::C, ContValue::D, ContValue::X, ContValue::Y, ContValue::Z,
    ContValue::HS, ContValue::MM, ContValue::SH, ContValue::JS, ContValue::GV, ContValue::M, ContValue::DM,
};
...
const auto ContStrings = []() {
    std::vector<std::string> result;
    for (auto&& i : ContUsageOrder) {
        result.push_back(ContArray[static_cast<int>(i)].first);
    }
    return result;
}();

const auto ValueStrings = []() {
    std::vector<std::string> result;
...
void ContComposerPanel::OnCmdTextEnterKeyPressed(wxCommandEvent& event)
{
    ...
    if (mCurrentSelected) {
        ...
        case TokenType::Do_Continuity: {
            ...
            auto theEntry = std::find_if(std::begin(ContArray), std::end(ContArray), [the_string](auto&& entry) {
                return the_string == entry.first;
            });
            if (theEntry == std::end(ContArray)) {
                theEntry = std::find_if(std::begin(ContArray), std::end(ContArray), [the_string](auto&& entry) {
                    return string_matches_upper_case(the_string, entry.first);
                });
            }
            if (theEntry != std::end(ContArray)) {
        ...
        case TokenType::Do_Values:
        case TokenType::Do_ValuesVarUnset:
            // unset variables make this complicated.
            // in the Set express, only Vars can be used, not just any value.
            // we try to set with the value, and tell the user if it's not valid.
            try {
                // special, if it just numbers, then it's a float.
                // if it's one of the directions, then it's that.
                double value;
                ...
                auto theEntry = std::find_if(std::begin(ValueMap), std::end(ValueMap), [the_string](auto&& entry) {
                    return the_string == entry.first;
                });
                if (theEntry == std::end(ValueMap)) {
                    theEntry = std::find_if(std::begin(ValueMap), std::end(ValueMap), [the_string](auto&& entry) {
                        return string_matches_upper_case(the_string, entry.first);
                    });
                }
                if (theEntry != std::end(ValueMap)) {
            ...
            break;
        case TokenType::Do_Points: {
            auto theEntry = std::find_if(std::begin(PointMap), std::end(PointMap), [the_string](auto&& entry) {
                return the_string == entry.first;
            });
            if (theEntry == std::end(PointMap)) {
                theEntry = std::find_if(std::begin(PointMap), std::end(PointMap), [the_string](auto&& entry) {
                    return string_matches_upper_case(the_string, entry.first);
                });
            }
            if (theEntry != std::end(PointMap)) {
...
void ContComposerPanel::UpdatePanel()
{
    ...
    //    mComboSelection->Popup();

    Refresh();
}

void ContComposerPanel::OnDrawableContClick(CalChart::DrawableCont const& c)
{
...
bool ContComposerPanel::Validate()
{
    return first_unset(mDrawableCont).first == nullptr;
}

std::unique_ptr<CalChart::ContProcedure>
ContComposerPanel::GetContinuity()
{
    return mCont->clone();
}

BEGIN_EVENT_TABLE(ContComposerDialog, wxDialog)
END_EVENT_TABLE()

IMPLEMENT_CLASS(ContComposerDialog, wxDialog)

ContComposerDialog::ContComposerDialog(std::unique_ptr<CalChart::ContProcedure> starting_continuity,
    ...
    : wxDialog(parent, id, caption, pos, size, style)
{
    ...
    mPanel = new ContComposerPanel(std::move(starting_continuity), CalChartConfiguration::GetGlobalConfig(), this);
...
ContComposerDialog::~ContComposerDialog() = default;

std::unique_ptr<CalChart::ContProcedure>
ContComposerDialog::GetContinuity()
{
    return mPanel->GetContinuity();
}

bool ContComposerDialog::Validate()
{
